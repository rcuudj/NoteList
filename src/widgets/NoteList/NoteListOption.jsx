import './NoteListOptions.css'
const NoteListOption = ({ handleShowList, isList, length }) => {
    return (
        <>
            <div className="note__list-options">
                <div className="note__list-options container">
                    <h2>{length > 0 ? `Все заметки (${length})` : `Нет заметок`}</h2>
                    <button className="note__list-option-button" onClick={() => { handleShowList() }}>
